...
export function LandReviewPage() {
  ...
  return <div className="min-h-screen bg-[#f3f3ec] px-4 py-6 text-[#15240a] sm:px-8"><main className="mx-auto max-w-[720px] space-y-5">
    <div className="flex items-center justify-between"><Link to={`/farmer/lands/${landId}`} className="flex items-center gap-2 text-sm font-semibold"><ArrowLeft size={15}/>Kembali</Link><span className="rounded-full bg-white px-3 py-1 text-[10px] font-semibold">Pertanyaan 1 dari 3 Pertanyaan</span></div>
    <div className="h-2 overflow-hidden rounded-full bg-[#deded4]"><motion.div initial={{width:0}} animate={{width:"33%"}} className="h-full bg-[#85c254]"/></div>
