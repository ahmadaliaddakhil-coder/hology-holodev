...
import { Check, ChevronLeft, ChevronRight, Minus, Plus, Sprout, Sun, Waves } from "lucide-react";
...
export function CropContextPage() {
  ...
  return <div className="min-h-screen bg-[#f3f3ec] text-[#15240a]">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[208px] flex-col justify-between bg-[#fafaf6] px-5 py-4 shadow-sm lg:flex">
      <div><div className="mb-5 rounded-xl bg-[#15240a]/80 px-3 py-2 text-[10px] font-bold tracking-[0.16em] text-white"><span className="text-[#85c254]">●</span> REMBUKTANI</div><Link to="/farmer/lands/new" className="mb-5 flex items-center justify-center gap-2 rounded-xl bg-[#85c254] px-3 py-3 text-sm font-bold">+ Tambah Lahan</Link><nav className="space-y-1"><SideLink to="/farmer/dashboard" icon={<Sprout size={17}/>} label="Beranda"/><SideLink to="/farmer/lands" icon={<Sprout size={17}/>} label="Lahan" active/><SideLink to="/farmer/history" icon={<Sprout size={17}/>} label="Riwayat"/><SideLink to="/farmer/profile" icon={<Sprout size={17}/>} label="Profil"/></nav></div>
      <div className="space-y-3"><div className="rounded-xl bg-[#e9fcb5] p-3 text-xs"><p className="font-bold">Sinkronisasi BMKG</p><p className="mt-1 text-[#44483f]">Data cuaca aktif</p></div><div className="flex items-center gap-2 px-1 text-xs"><span className="flex size-8 items-center justify-center rounded-full bg-[#0d1b03] text-white">●</span><div><p className="font-bold">Akun aktif</p><p className="text-[#666a60]">Petani</p></div></div></div>
    </aside>
    <div className="lg:pl-[208px]"><header className="flex h-16 items-center justify-between border-b border-[#deded4]/70 bg-[#fafaf6] px-4 sm:px-8"><span className="rounded bg-[#e4f6b0] px-3 py-1 text-xs font-bold">Wilayah: {location}</span><span className="hidden items-center gap-2 text-xs font-semibold sm:flex"><Waves size={17}/> Cuaca lahan aktif</span></header>
      <main className="mx-auto max-w-[980px] px-4 py-7 sm:px-8">
        <div className="mb-5 flex items-start justify-between gap-4"><div><Link to="/farmer/lands/new/location" className="flex items-center gap-2 text-xs font-bold text-[#56652e]"><ChevronLeft size={15}/>Kembali ke Pilih Lahan</Link><h1 className="mt-5 font-display text-3xl font-bold sm:text-4xl">Informasi Tanaman & Umur Padi</h1><p className="mt-2 max-w-2xl text-sm text-[#56652e]">Menentukan kebutuhan air, keamanan pemupukan, dan ketahanan terhadap prakiraan cuaca BMKG.</p></div><span className="mt-8 hidden rounded-full bg-[#e4f6b0] px-3 py-2 text-xs font-bold md:block"><span className="text-[#85c254]">●</span> Langkah 2 dari 2</span></div>
