...
export class LlmReasoningEnhancer {
  ...
  async enhance(input:ReasoningInput, baseline:ReasoningAssessment, crop:CropContext|null):Promise<EnhancedAssessment>{
    ...
    try{
      ...
      const prompt=`Anda adalah lapisan penjelasan RembukTani, bukan pembuat tindakan, pengambil keputusan, atau agronom otonom. Rule engine sudah memilih action catalog pada rule_selected_actions. Jangan membuat summary kondisi. Jangan menambah, menghapus, mengganti, mengurutkan berdasarkan kualitas, atau mengubah option_id maupun title. Kembalikan tepat satu explanation untuk setiap option_id yang diberikan, dalam urutan yang sama. Anda hanya boleh memperjelas description dan rationale berdasarkan JSON berikut. Jangan menciptakan data, prediksi hasil, dosis, volume, durasi, jadwal presisi, diagnosis hama, instruksi wajib, atau klaim kepastian. Jangan gunakan bahasa mengarahkan seperti disarankan, sebaiknya, segera, wajib, atau harus. Sebutkan keterbatasan secara jujur. Keputusan akhir milik petani.\nDATA=${JSON.stringify(safeInput)}`;
