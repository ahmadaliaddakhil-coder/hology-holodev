...
export function SelectReviewerPage() {
  ...
  const location = data ? [data.land.village, data.land.regency].filter(Boolean).join(", ") : "Wilayah Kecamatan Kepanjen";
  ...
  return <div className="min-h-screen bg-[#f3f3ec] text-[#15240a]">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[260px] flex-col justify-between bg-[#fafaf6] p-5 shadow-sm transition-transform duration-300 ${mobileNavOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
      ...
      <div className="space-y-4"><div className="rounded-xl bg-[#e9fcb5] p-3"><p className="text-xs font-bold"><span className="mr-1.5 inline-block size-2 rounded-full bg-[#85c254]" />Sinkronisasi BMKG</p><p className="pl-4 text-[10px] text-[#56652e]">Data cuaca aktif</p></div><div className="flex items-center justify-between px-1"><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-full bg-[#0d1b03] text-white"><UserCircle2 size={15} /></span><div><p className="text-xs font-bold">{user?.display_name || user?.displayName || "Pak Slamet"}</p><p className="text-[10px] text-[#666a60]">Ketua Poktan</p></div></div><Settings size={15} className="text-[#666a60]" /></div></div>
    </aside>

    <div className="md:ml-[260px]"><header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-[#deded4]/50 bg-[#f3f3ec]/90 px-4 backdrop-blur-md sm:px-8"><button type="button" className="rounded-md p-1.5 md:hidden" onClick={() => setMobileNavOpen(true)} aria-label="Buka menu"><Menu size={20} /></button><span className="truncate rounded bg-[#e9fcb5] px-2.5 py-1 text-[10px] font-bold text-[#213014] sm:text-xs">Wilayah: {location}</span><div className="flex items-center gap-4"><span className="hidden items-center gap-1.5 text-xs font-medium sm:flex"><CloudSun size={16} /> Cerah Berawan 28°C</span><Bell size={17} /><span className="flex size-7 items-center justify-center rounded-full bg-[#0d1b03] text-white"><UserCircle2 size={14} /></span></div></header>
